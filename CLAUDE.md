...
- **Caída en producción (2026-09-14) — `Cannot find module '@swc/helpers/_/_interop_require_default'`**: el sitio tiraba 500 en todas las rutas al iniciar. `@swc/helpers` sí estaba correctamente declarado como dependencia transitiva de `next` en `pnpm-lock.yaml` (no era un problema de lockfile) — la causa real es que pnpm arma `node_modules` con symlinks, y el paso interno del pipeline de Hostinger que mueve el build de `.builds/source/repository/` a la carpeta real del sitio (`nodejs/`) no los preserva de forma confiable, así que el módulo "desaparece" en producción aunque esté bien instalado. Solución: `.npmrc` en la raíz con `node-linker=hoisted`, que hace que pnpm instale con archivos reales en vez de symlinks (estructura plana, tipo npm) — así sobrevive a cualquier copia entre directorios que haga Hostinger. También se agregó `@swc/helpers` como dependencia directa en `package.json` (antes solo venía transitiva por `next`) como medida defensiva adicional. Si vuelve a aparecer este mismo error tras un deploy, lo primero a revisar es que `.npmrc` siga en el repo y no se haya perdido.
